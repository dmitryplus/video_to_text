...
Видео скачивается с Яндекс.Диска или YouTube, вырезается аудио дорожка и прогоняется через модель для распознавания речи `ai-sage/GigaAM-v3`.
...
- Доступ в интернет для скачивания видеофайла с Яндекс.Диска или YouTube и первой загрузки моделей.
...
```bash
./.venv/bin/python main.py "https://www.youtube.com/watch?v=neHSPe-P408" --output youtube_transcript.json
```
...
- URL на видео поддерживаются через `file_processing.py`: Яндекс.Диск скачивается через прямую ссылку, YouTube скачивается через `yt-dlp` как видеофайл до 480p, дальше из него извлекается и обрабатывается аудиодорожка.
