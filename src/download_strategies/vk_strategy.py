...
class VkVideoDownloadStrategy(VideoDownloadStrategy):
    def can_handle(self, source_url: str) -> bool:
        parsed = urlparse(source_url)
        host = parsed.netloc.lower()
        return any(domain in host for domain in ("vkvideo.ru", "vk.com"))
    ...
    def download(self, source_url: str, target_dir: Path, preferred_stem: str) -> Path:
        ...
        def progress_hook(event: dict) -> None:
            ...
            if status == "downloading":
                if progress_bar is None or total != progress_total:
                    progress_bar = ProgressBar(total=total, label="[VK Video]")
        ...
        self._log("Скачиваю видео с VK Video")
        options = {
            "format": VK_VIDEO_FORMAT,
        ...
        try:
            ...
            raise SystemExit(f"Не удалось скачать видео с VK Video: {exc}") from exc
        ...
        self._log(f"Видео VK Video скачано: {downloaded_path}")
